...
const CELL_PARA = { spacing: { after: 0, line: 264 } };
      ...
      (c) =>
        new TableCell({
          ...
          children: [
            new Paragraph({
              ...CELL_PARA,
              children: [new TextRun({ text: c, bold: true, color: "1F3F6B" })],
            }),
          ],
        })
    ),
  ...
  return new TableRow({
    children: cells.map(
      (c) =>
        new TableCell({
          margins: CELL_MARGINS,
          children: [new Paragraph({ ...CELL_PARA, text: c || "" })],
        })
    ),
  });
}
    ...
    styles: {
      default: {
        document: {
          ...
          // Generous spacing after each paragraph and 1.15 line height so the
          // report no longer reads as cramped.
          paragraph: { spacing: { after: 180, line: 276 } },
        },
        title: {
          run: { size: 40, bold: true, color: "1F3F6B" },
          paragraph: { spacing: { before: 60, after: 200 } },
        },
        heading1: {
          run: { size: 28, bold: true, color: "1F3F6B" },
          paragraph: { spacing: { before: 320, after: 140 } },
        },
        heading2: {
          run: { size: 24, bold: true, color: "1F3F6B" },
